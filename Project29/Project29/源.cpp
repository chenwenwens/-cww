...
#define Frame_height  22 //游戏窗口的高度
...
void make_frame()
{
	...
	//添加背景音乐--TaylorSwift--YOU NEED TO CALM DOWN
	PlaySound("taylor.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	system("color B0"); //改变背景为淡浅绿色，文字为黑色
...
{       //当某行有Frame_width-2个方块时，则满行
	...
	if (del_count > 0 && (tetris->score % 600 == 0 || tetris->score / 600 > tetris->level - 1))
	{        //如果得600分即累计删除6行，速度加快20ms并升一级
		tetris->speed -= 20;
		tetris->level++;
	}
}
